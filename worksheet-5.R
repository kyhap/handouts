...
library(dplyr)
library(ggplot2)
animals <- read.csv("animals.csv", na.strings = '') %>%
...
ggplot(data=animals_dm, aes(x=year, y=weight), color=sex) + geom_line(stat='summary', fun.y='mean')
...
## Exercise 2
ggplot(data=animals_dm, aes(x=weight, fill=sex)) +geom_histogram(binwidth= 1)
...
ggplot(data=animals_common, aes(x=weight, fill=species_id))+geom_histogram(aes(y=..density..))+
